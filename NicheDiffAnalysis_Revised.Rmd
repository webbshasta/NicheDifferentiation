...
#### Rachel Williamson, Shasta Webb, Amanda Melin
...
library(readr); library(dplyr); library(car); library(ggplot2); library(lme4);  library(car); library(emmeans); library(multcomp); library(multcompView); library(gridExtra); library(lubridate); library(fuzzyjoin); library(ggpubr); library(glmmTMB); library(performance); library(effects); library(sjPlot)
```
...
```{r}
rotationTable <- as.data.frame(read.csv("~/Desktop/Manuscripts/Submitted/NicheDifferentiation/SupportingDocs/rotationDate16_17.csv"))
rotationTable <- distinct(rotationTable)
```

##### Reading in birth date data

```{r}
birthDate <-  as.data.frame(read.csv("~/Desktop/Manuscripts/Submitted/NicheDifferentiation/SupportingDocs/Birthdate_SSR.csv"))
```

##### Reading in monthly fruit biomass data

```{r}
fruitBiomass <- as.data.frame(read.csv("~/Desktop/Manuscripts/Submitted/NicheDifferentiation/SupportingDocs/MonthlyFruitBiomass_copy.csv"))

#creating a month/year column for left joining to behavioural data
fruitBiomass$Month_Yr <- format(as.Date(fruitBiomass$YearOf), "%Y-%m")
```
...
df.summary <- as.data.frame(read.csv("~/Desktop/Manuscripts/Submitted/NicheDifferentiation/SupportingDocs/IndividScanStatesXDays.csv"))
names(df.summary)
...
df.summary <- df.summary %>%
                    ...
                    mutate(OtherFoodScans = TotalForagingScans-(FruitScans + InvScans))
...
names(support.summary)

support.inv <- support.summary %>%
               drop_na(Smbr) %>%
               group_by(AgeSexClass) %>%
               summarise(Ground = sum(GRO),
                         Low = sum(LOW),
                         Mid = sum(MID),
                         Upper = sum(UPP),
                ...
                mutate(Ground_prop = Ground/TotalInvVertScans,
                       Low_prop = Low/TotalInvVertScans,
                       Mid_prop = Mid/TotalInvVertScans,
                       Upp_prop = Upper/TotalInvVertScans,
                       Sm_prop = SmBranch/TotalInvSuppScans,
                       Md_prop = MdBranch/TotalInvSuppScans,
                       Lg_prop = LgBranch/TotalInvSuppScans,
                       Hori_prop = Hori/TotalInvSuppScans,
                       Vert_prop = Vert/TotalInvSuppScans)
...
support.fruit <- summary.fruit %>%
               ...
               summarise(Ground = sum(GRO),
                ...
                mutate(Ground_prop = Ground/TotalFruVertScans,
                       Low_prop = Low/TotalFruVertScans,
                       Mid_prop = Mid/TotalFruVertScans,
                       Upp_prop = Upper/TotalFruVertScans,
                       Sm_prop = SmBranch/TotalFruSuppScans,
                       Md_prop = MdBranch/TotalFruSuppScans,
                       Lg_prop = LgBranch/TotalFruSuppScans,
                       Hori_prop = Hori/TotalFruSuppScans,
                       Vert_prop = Vert/TotalFruSuppScans)
...
```{r}
df.overallStates <- as.data.frame(read.csv("~/Desktop/Manuscripts/Submitted/NicheDifferentiation/SupportingDocs/IndividScanStatesXDays.csv"))

df.overallStates$Group <- as.factor(df.overallStates$Group)
df.overallStates$AdjustedAge <- as.factor(df.overallStates$AdjustedAge)
df.overallStates$AdjustedAgeSex <- as.factor(df.overallStates$AdjustedAgeSex)
...
df.overallStates$Sex.Class <- factor(df.overallStates$Sex.Class, levels = c("female", "male"))
```
...
```{r}
sum(df.overallStates$TotalScans) #30744 foraging scans collected
...
df.overallStates <- df.overallStates %>%
                    ...
                    dplyr::summarise(Group = unique(Group),
                              ...
                              AgeClass = mode(AdjustedAge),
                              SexClass = unique(Sex.Class),
...
df.overallStates <- filter(df.overallStates, TotalForagingScans > 0)
```
...
```{r}
dfh1.difficulty <- as.data.frame(read.csv("~/Desktop/Manuscripts/Submitted/NicheDifferentiation/SupportingDocs/FruitDifficultyScansXDays.csv"))

dfh1.difficulty$Group <- as.factor(dfh1.difficulty$Group)
dfh1.difficulty$AdjustedAge <- as.factor(dfh1.difficulty$AdjustedAge)
dfh1.difficulty$AdjustedAgeSex <- as.factor(dfh1.difficulty$AdjustedAgeSex)
...
Filtering for individuals who were only in study <2 months, and who had very few scans
...
```{r}
dfh1.invertebrates <- as.data.frame(read.csv("~/Desktop/Manuscripts/Submitted/NicheDifferentiation/SupportingDocs/IndividScanStatesXDays.csv"))

dfh1.invertebrates$Group <- as.factor(dfh1.invertebrates$Group)
dfh1.invertebrates$AdjustedAge <- as.factor(dfh1.invertebrates$AdjustedAge)
dfh1.invertebrates$AdjustedAgeSex <- as.factor(dfh1.invertebrates$AdjustedAgeSex)
...
dfh1.invertebrates$Sex.Class <- factor(dfh1.invertebrates$Sex.Class, levels = c("female", "male"))
```
...
dfh1.invertebrates <- dfh1.invertebrates %>%
                      group_by(Rotation, Animal) %>%
                      summarise(AdjustedAgeSex = mode(AdjustedAgeSex),
                              AgeClass = mode(AdjustedAge),
                              Month_Yr = mode(Month_Yr),
                              SexClass = unique(Sex.Class),
                              Group = unique(Group),
...
```{r}
supportDF.invertebrates <- as.data.frame(read.csv("~/Desktop/Manuscripts/Submitted/NicheDifferentiation/SupportingDocs/IndividHabitatInvertebrateScansXDays.csv"))

supportDF.invertebrates$Group <- as.factor(supportDF.invertebrates$Group)
supportDF.invertebrates$AdjustedAge <- as.factor(supportDF.invertebrates$AdjustedAge)
supportDF.invertebrates$AdjustedAgeSex <- as.factor(supportDF.invertebrates$AdjustedAgeSex)
...
plot_model(branchUse.invertebrate.model,  
           ...
           colors = c("mediumpurple2", "orange", "burlywood4")) +
           theme_minimal() +
           ylab("\nIncidence Rate Ratios \n(Small Branch Scans per Rotation\nduring Invertebrate Foraging Scans)\n") -> branchUse.invertebrate.model.plot; branchUse.invertebrate.model.plot
...
ggsave(plot = combined_SmBranch_InvModel, filename = "~/Desktop/Manuscripts/Submitted/NicheDifferentiation/Figures/combined_SmBranch_InvModel.pdf", width = 10, height = 4)
```

Preparing data for fruit foraging & branch use

```{r}
supportDF.fruit <- as.data.frame(read.csv("~/Desktop/Manuscripts/Submitted/NicheDifferentiation/SupportingDocs/IndividHabitatFruitScansXDays.csv"))
supportDF.fruit$Group <- as.factor(supportDF.fruit$Group)
supportDF.fruit$AdjustedAge <- as.factor(supportDF.fruit$AdjustedAge)
supportDF.fruit$AdjustedAgeSex <- as.factor(supportDF.fruit$AdjustedAgeSex)
...
```{r}
rotationTable2017 <- as.data.frame(read.csv("~/Desktop/Manuscripts/Submitted/NicheDifferentiation/SupportingDocs/RotationsREW.csv"))

rotationTable2017 <- rotationTable2017 %>%
                     dplyr::select(DateOf, Rotation) %>%
                     rename(Date = DateOf)
```
...
supportDF.fruit <- supportDF.fruit %>%
                   group_by(Rotation, Animal) %>%
                   summarise(Month_Yr = mode(Month_Yr),
                             AdjustedAgeSex = mode(AdjustedAgeSex),
                             AgeClass = mode(AdjustedAge),
                             SexClass = unique(SexClass),
                             Group = mode(Group),
                             MeanTempMax = mean(TemperatureMax),
...
raw.vertical.invertebrate.plot <- ggplot(data = supportDF.invertebrates, aes(x = AgeClass, y = VerticalScans, fill = SexClass)) +
                         ...
                         scale_x_discrete(limit = c("infant", "small immature", "large immature", "adult"),
                                          labels = c("Infant", "Small\nImmature", "Large\nImmature", "Adult")) +
                         xlab("") + 
                         ylab("\nVertical Branch Scans during Invertebrate Foraging\n(Raw Counts per Rotation)\n") +
                         geom_jitter(alpha = 5/10, height = 0, pch = 1) +
                         theme_minimal() +
                         theme(legend.position = "bottom",
...
raw.vert.fruit.plot <- ggplot(data = supportDF.fruit, aes(x = AgeClass, y = VerticalScans, fill = SexClass)) +
                         ...
                         scale_x_discrete(limit = c("infant", "small immature", "large immature", "adult"),
                                          labels = c("Infant", "Small\nImmature", "Large\nImmature", "Adult")) +
                         xlab("") + 
                         ylab("\nVertical Branch Scans during Fruit Foraging\n(Raw Counts per Rotation)\n") +
                         geom_jitter(alpha = 5/10, height = 0, pch = 1) +
                         theme_minimal() +
                         theme(legend.position = "bottom",
...
```{r}
supportDF.invertebrates.arboreal <- as.data.frame(read.csv("~/Desktop/Manuscripts/Submitted/NicheDifferentiation/SupportingDocs/IndividHabitatInvertebrateScansXDays.csv"))

supportDF.invertebrates.arboreal$Group <- as.factor(supportDF.invertebrates.arboreal$Group)
supportDF.invertebrates.arboreal$AdjustedAge <- as.factor(supportDF.invertebrates.arboreal$AdjustedAge)
supportDF.invertebrates.arboreal$AdjustedAgeSex <- as.factor(supportDF.invertebrates.arboreal$AdjustedAgeSex)
...
```{r}
supportDF.fruits.arboreal <- as.data.frame(read.csv("~/Desktop/Manuscripts/Submitted/NicheDifferentiation/SupportingDocs/IndividHabitatFruitScansXDays.csv")) 

supportDF.fruits.arboreal$Group <- as.factor(supportDF.fruits.arboreal$Group)
supportDF.fruits.arboreal$AdjustedAge <- as.factor(supportDF.fruits.arboreal$AdjustedAge)
supportDF.fruits.arboreal$AdjustedAgeSex <- as.factor(supportDF.fruits.arboreal$AdjustedAgeSex)
...
```{r}
supportDF.invertebrates.all <- as.data.frame(read.csv("~/Desktop/Manuscripts/Submitted/NicheDifferentiation/SupportingDocs/IndividHabitatInvertebrateScansXDays.csv")) 

supportDF.invertebrates.all$Group <- as.factor(supportDF.invertebrates.all$Group)
supportDF.invertebrates.all$AdjustedAge <- as.factor(supportDF.invertebrates.all$AdjustedAge)
supportDF.invertebrates.all$AdjustedAgeSex <- as.factor(supportDF.invertebrates.all$AdjustedAgeSex)
...
```{r}
groundUse.invert.model <- glmer(GroundScans ~ AdjustedAge +
                                      SexClass +
                                      (AdjustedAge*SexClass) +
                                      MeanRainfall +
                                      #MeanTempMax +
                                      #MonthlyFruitBiomass_kg_ha +
...
plot_model(groundUse.invert.model.zi,  
           ...
           group.terms = c(1,1,1,2,3,3,3),
           colors = c("mediumpurple2", "orange", "burlywood4")) +
           ylab("\nIncidence Rate Ratios\n)Ground Use Scan Frequency during\nInvertebrate Foraging per Rotation\n") +
...
ggsave(plot = combined_ground_inv, filename = "~/Desktop/Manuscripts/Submitted/NicheDifferentiation/Figures/combined_ground_inv.pdf", width = 10, height = 4)
```
...
```{r}
supportDF.fruit.all <- as.data.frame(read.csv("~/Desktop/Manuscripts/Submitted/NicheDifferentiation/SupportingDocs/IndividHabitatFruitScansXDays.csv")) 

supportDF.fruit.all$Group <- as.factor(supportDF.fruit.all$Group)
supportDF.fruit.all$AdjustedAge <- as.factor(supportDF.fruit.all$AdjustedAge)
supportDF.fruit.all$AdjustedAgeSex <- as.factor(supportDF.fruit.all$AdjustedAgeSex)
...
raw.ground.fruit.plot <- ggplot(data = supportDF.fruit.all, aes(x = AgeClass, y = GroundScans, fill = SexClass)) +
                         ...
                         scale_x_discrete(limit = c("infant", "small immature", "large immature", "adult"),
                                          labels = c("Infant", "Small\nImmature", "Large\nImmature", "Adult")) +
                         xlab("") + 
                         ylab("\nGround use Scans during Fruit Foraging\n(Raw Counts per Rotation )\n") +
                         geom_jitter(alpha = 5/10, height = 0, pch = 1) +
                         theme_minimal() +
                         theme(legend.position = "bottom",
...
```{r}
groundUse.fruit.model <- glmer(GroundScans ~ AdjustedAge +
                                      SexClass +
                                      (AdjustedAge*SexClass) +
                                      #MeanRainfall +
                                      #MeanTempMax +
